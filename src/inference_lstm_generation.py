...
def main(args):
    ...
    total_words, max_sequence_len = read_parameters("data/processed/lstm_parameters.txt")
    model = tf.keras.models.load_model("models/lstm/lstm_lyrics_generator.h5")
    output = complete_the_song(seed_text=args.seed_text, next_words=args.next_words, tokenizer=tokenizer, model=model, max_sequence_len=max_sequence_len)
    print(output)
